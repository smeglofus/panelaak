...
describe('calendar', () => {
  it('starts on 1. dubna 1988', () => {
    expect(dateFromTick(0)).toEqual({ year: 1988, month: 4, day: 1 });
    expect(formatDateCs(dateFromTick(0))).toBe('1. dubna 1988');
  });

  it('advances one day per 30 seconds', () => {
    expect(dateFromTick(SECONDS_PER_DAY)).toEqual({ year: 1988, month: 4, day: 2 });
    expect(dateFromTick(30 * SECONDS_PER_DAY)).toEqual({ year: 1988, month: 5, day: 1 });
  });

  it('rolls over the year', () => {
    const newYear = dateFromTick(270 * SECONDS_PER_DAY); // 270 days after 1.4. = 1.1.
    expect(newYear).toEqual({ year: 1989, month: 1, day: 1 });
  });
